...
public class SampleXmlUtilTestCaseData
{
    private boolean _soapEnc = true;
    ...
    private SampleXmlUtilTestCaseData(boolean soapEnc)
    {
        _soapEnc = soapEnc;
    }
    ...
    public static String createSampleForType(SchemaType sType)
    {
        ...
        new SampleXmlUtilTestCaseData(true).createSampleForType(sType, cursor);
    ...
    private void createSampleForType(SchemaType stype, XmlCursor xmlc)
    {
        if(stype.getName()!= null){
            ...
            SchemaProperty[] elementProps = stype.getElementProperties();
            if (elementProps.length>0){
                elementName= elementProps[0].getName().getLocalPart();
            }
 ...
 */
                elementPathList.remove(elementPathList.size()-1);
            }
 ...
 */
                elementPathTypeList.remove(elementPathTypeList.size()-1);
            }
...
//X            else System.out.println("   --** Did not Decrease Level as planned");
    ...
    private void processSimpleType(SchemaType stype, XmlCursor xmlc)
    {
        ...
        stype.getFacet(SchemaType.FACET_MIN_INCLUSIVE);
        if (elementPathList.size()>0)System.out.println(elementPathList.get(elementPathList.size()-1)+ "  "+optional+"  value="+ sample+"   Type? "+elementPathTypeList.get(elementPathTypeList.size()-1)+"   Simple Element Type => " + dataTypeForSimpleType(stype)+ "   Message ="+messageName+"   Current Path =" + previousPath);
    ...
    private String dataTypeForSimpleType(SchemaType sType)
    {
        if (XmlObject.type.equals(sType))
            return "anyType";
        ...
        switch (sType.getPrimitiveType().getBuiltinTypeCode())
        {
            ...
            case SchemaType.BTC_G_DAY:
            case SchemaType.BTC_G_MONTH:
                return sType.getPrimitiveType().toString();
        }
    }
    ...
    */
    ...
    private void processElement(SchemaParticle sp, XmlCursor xmlc, boolean mixed)
    {
        ...
        try{
            ...
            if(elementPathList.size()>0)parent = elementPathList.get(elementPathList.size()-1);
            if(elementPathTypeList.size()>0)parentType = elementPathTypeList.get(elementPathTypeList.size()-1);
//            System.out.println(pad+"processElement => "+element.getName().getLocalPart()+ "   Prefix="+element.getType().getSourceName()+"    type "+element.getType().getName().getLocalPart()+ "   @level"+level + "    Message="+messageName+"   Parent =" + parent+"   ParentType"+parentType+"   Path= "+previousPath);
  //XXX          System.out.println(element.getName().getLocalPart()+ ","+element.getType().getSourceName()+","+element.getType().getName().getLocalPart()+ ","+level + ","+messageName+"," + parent+","+parentType+","+previousPath);
//            if (level >=0){
...
//X                System.out.println("   +++ Adding Element "+ element.getName().getLocalPart()+ " @ Array location "+ (elementPathList.size()-1)+ " with level ="+level);
                if (elementPathList.size()>0){
                    if (!((elementPathList.get(elementPathList.size()-1)).equals(element.getName().getLocalPart())))elementPathList.add(element.getName().getLocalPart());
                } else{
                    elementPathList.add(element.getName().getLocalPart());
                }
                if (elementPathTypeList.size()>0){
                    if (!((elementPathTypeList.get(elementPathTypeList.size()-1)).equals(element.getName().getLocalPart())))elementPathTypeList.add(element.getType().getName().getLocalPart());
                } else{
...
//              nex.printStackTrace();
 //XXX           System.out.println(element.getName().getLocalPart()+ ","+element.getType().getSourceName()+","+"***TypeError***"+ ","+level + ","+messageName+"," + parent+","+parentType+","+previousPath);
            if (level >=0){
//                elementPathList.add(level, element.getName().getLocalPart());
//                elementPathTypeList.add(level, element.getType().getName().getLocalPart());
//X                System.out.println("   +++ Adding Element "+ element.getName().getLocalPart()+ " @ Array location "+ (elementPathList.size()-1)+ " with level ="+level);
                ...
                if (elementPathTypeList.size()>0){
                    if (!((elementPathTypeList.get(elementPathTypeList.size()-1)).equals("***UnresolvableType****")))elementPathTypeList.add("***UnresolvableType****");
                } else{
...
//                elementPathTypeList.add(level, element.getType().toString());
                ...
                if (elementPathTypeList.size()>0){
                    if (!((elementPathTypeList.get(elementPathTypeList.size()-1)).equals("***UnresolvableType****")))elementPathTypeList.add(element.getType().toString());
                } else{
    ...
    private void processChoice(SchemaParticle sp, XmlCursor xmlc, boolean mixed)
    {
        ...
        int choiceItemSelected = 0;
        if (spc.length > 0) choiceItemSelected = _picker.nextInt(spc.length);
        xmlc.insertComment("You have a CHOICE of the next " + String.valueOf(spc.length) + " items at this level.  Item "+choiceItemSelected+" was chosen.");
        processParticle(spc[choiceItemSelected], xmlc, mixed);
/**
        for (int i=0; i < spc.length; i++)
        {
            processParticle(spc[i], xmlc, mixed);
