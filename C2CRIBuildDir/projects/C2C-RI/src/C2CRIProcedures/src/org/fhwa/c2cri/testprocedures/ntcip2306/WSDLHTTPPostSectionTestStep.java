 ...
 */
public class WSDLHTTPPostSectionTestStep extends TestProcedureStep {

    private NTCIP2306Specification relatedSpecification;

    public WSDLHTTPPostSectionTestStep() {
        this.setDescription("IF NTCIP2306_N2R7_WSDL_Request_Response_HTTP_POST_Flag is equal to True CONTINUE; OTHERWISE skip the following substeps.");
        this.setResults("NA");
        ...
        try{
            ...
            for (NTCIP2306Specification thisSpecification : wsdlSpecificationList) {
                if (thisSpecification.getSection().startsWith("8.2")){
...
//                   newStep.setPrimaryStepId(getId());
    ...
    public void setStartStepNumber(Integer stepNumber) {
        System.out.println("WSDLHTTPPostSectionTestStep:  Step Number "+stepNumber);
        ...
        for (Section thisSubsection : getSubSteps()){
               ...
               System.out.println("WSDLHTTPPostSectionTestStep:  Set Primary Step Number to "+stepNumber+" for step "+((TestProcedureStep)thisSubsection).getId());
         }
...
//        theList.add(this);
    ...
    public String getScriptContent() {
        ...
        result = "<testStep functionId=\"Step "+this.getId()+" IF NTCIP2306_N2R7_WSDL_Request_Response_HTTP_POST_Flag is equal to True CONTINUE; OTHERWISE skip the following substeps.\"   passfailResult=\"False\" />\n<jl:if test=\"${NTCIP2306_N2R7_WSDL_Request_Response_HTTP_POST_Flag}\" >\n"+subSteps+"</jl:if>\n";
        return result;
    }
}
