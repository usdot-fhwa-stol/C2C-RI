 ...
 */
public class WSDLSOAPSubPubSectionTestStep extends TestProcedureStep {

    private NTCIP2306Specification relatedSpecification;

    public WSDLSOAPSubPubSectionTestStep() {
        this.setDescription("IF NTCIP2306_N1R2_WSDL_Subscription_Publication_Flag is equal to True CONTINUE; OTHERWISE skip the following substeps.");
        this.setResults("NA");
        ...
        try{
            ...
            for (NTCIP2306Specification thisSpecification : wsdlSpecificationList) {
                if (thisSpecification.getSection().startsWith("7.2")){
...
//                   newStep.setPrimaryStepId(getId());
    ...
    public void setStartStepNumber(Integer stepNumber) {
        System.out.println("WSDLSOAPSubPubSectionTestStep:  Step Number "+stepNumber);
        ...
        for (Section thisSubsection : getSubSteps()){
               ...
               System.out.println("WSDLSOAPSubPubSectionTestStep:  Set Primary Step Number to "+stepNumber+" for step "+((TestProcedureStep)thisSubsection).getId());
         }
...
//        theList.add(this);
    ...
    public String getScriptContent() {
        ...
        result = "<testStep functionId=\"Step "+this.getId()+" IF NTCIP2306_N1R2_WSDL_Subscription_Publication_Flag is equal to True CONTINUE; OTHERWISE skip the following substeps.\"   passfailResult=\"False\" />\n<jl:if test=\"${NTCIP2306_N1R2_WSDL_Subscription_Publication_Flag}\" >\n"+subSteps+"</jl:if>\n";
        return result;
    }
}
