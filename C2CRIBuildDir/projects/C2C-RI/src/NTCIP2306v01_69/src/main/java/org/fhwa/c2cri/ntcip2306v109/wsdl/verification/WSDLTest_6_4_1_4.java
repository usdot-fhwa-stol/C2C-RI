...
/**
 * The Class WSDLTest_6_4_1_4.
 *
 * @author TransCore ITS, LLC
 * Last Updated:  1/8/2014
 */
public class WSDLTest_6_4_1_4 implements WSDLTest {
    ...
    /**
     * Instantiates a new wSDL test_6_4_1_4.
     * 
     * Pre-Conditions: N/A
     * Post-Conditions: N/A
     *
     ...
     */
    public WSDLTest_6_4_1_4(RIWSDL wsdlSpec, String wsdlURL) {
    ...
    public TestAssertion perform() {
        ...
        assertion.setTestAssertionID("6.4.1.4");
        ...
        try {
            ...
            for (String thisDialogMessage : dialogMessageList) {
                if (definedMessageList.contains(thisDialogMessage)) {
                    ...
                    while (xmlc.hasNextSelection()) {
                        xmlc.toNextSelection();
                        // Confirm the prefix as well as the element part with the schema
                        String prefix = xmlc.getTextValue().substring(0, xmlc.getTextValue().indexOf(":"));
                        String schemaElement = xmlc.getTextValue().substring(xmlc.getTextValue().indexOf(":")+1);
                        boolean matchedToSchema = false;
                        for (QName thisQName : schemaMessageList) {

                            if ((thisQName.getLocalPart().equals(schemaElement)) && (thisQName.getPrefix().equals(prefix))) {
                                matchedToSchema = true;
                                break;
                            }
                        }
                        if (!matchedToSchema) {
                            incorrectAttributeMessages.add(thisDialogMessage);
                        }
        ...
        assertion.setTestAssertionPrescription("The element attribute of the message part name shall use the form tmdd: followed by the name of the XML Schema element being imported into the WSDL. ");
        return assertion;
    }
}
