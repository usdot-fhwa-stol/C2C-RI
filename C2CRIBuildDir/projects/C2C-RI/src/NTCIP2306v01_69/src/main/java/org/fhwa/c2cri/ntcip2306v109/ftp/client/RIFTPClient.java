 ...
 **/
public class RIFTPClient extends FTPClient

{
    ...
    private Integer dataListenerID=-1;
      ...
      throws IOException
    {
        ...
        if (__dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE)
        {
            ServerSocket server;
    /**
     * Modified to facilitate RI Listener
     * 
     * Set up the server on the internal server port, but tell the FTP Server to connect to our
     * external port which is already set up by our Listener.
     */
            server = _serverSocketFactory_.createServerSocket(0,1,getLocalAddress());
            try{
            dataListenerID = ListenerManager.getInstance().createServerModeListener(server.getInetAddress().getHostAddress(),
                                            server.getLocalPort(), false, "FTP ACTIVE CLIENT DATA CONNECTION");
            } catch (TransportException tex){
                tex.printStackTrace();
                throw new IOException("Error opening the Server Mode Listner ",tex);
            }
            InetAddress thisIp;
            InetAddress localhostAddress;
            try {
                localhostAddress = InetAddress.getLocalHost();
                thisIp = localhostAddress.getByName(localhostAddress.getHostName());
            } catch (Exception e) {
                thisIp = getLocalAddress();
            }
        ...
        { // We must be in PASSIVE_LOCAL_DATA_CONNECTION_MODE
            ...
            String newSocketHost = "";
            int newSocketPort = -1;
            try{
               dataListenerID = ListenerManager.getInstance().createClientModeListener( __passiveHost, __passivePort, false, "FTP CLIENT CONNECTION PASSIVE");
               newSocketHost = ListenerManager.getInstance().getListenerInternalServerAddress(dataListenerID);
               newSocketPort = ListenerManager.getInstance().getListenerInternalServerPort(dataListenerID);

            } catch (Exception ex){
               throw new IOException("RIFTPClient:_openDataConnection "+ex.getMessage());
            }
             socket = _socketFactory_.createSocket(newSocketHost, newSocketPort);
            if ((__restartOffset > 0) && !restart(__restartOffset))
            {
                socket.close();
                return null;
    ...
    {
        if (ListenerManager.getInstance().listenerExists(dataListenerID)){
            ListenerManager.getInstance().stopListener(dataListenerID);
        }
        if (ListenerManager.getInstance().listenerExists(dataListenerID)){
            ListenerManager.getInstance().stopListener(dataListenerID);
        }
