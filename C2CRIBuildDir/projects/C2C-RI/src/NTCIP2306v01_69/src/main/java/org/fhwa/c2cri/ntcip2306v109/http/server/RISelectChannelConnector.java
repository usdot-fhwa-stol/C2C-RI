 ...
 * </p>
 *
 * Modified by the RI project to capture addresses and ports for logging.
 * Last updated: 10/13/2013 
 * @org.apache.xbean.XBean element="nioConnector" description="Creates an NIO based socket connector"
 */
public class RISelectChannelConnector extends AbstractNIOConnector
{
    ...
    private final SelectorManager _manager = new SelectorManager()
    {
        ...
        {
            return RISelectChannelConnector.this.newConnection(channel,endpoint);
        }
        ...
        {
            return RISelectChannelConnector.this.newEndPoint(channel,selectSet,sKey);
        }
    };
     ...
     *
     */
    public RISelectChannelConnector()
    {
        this.location = "";
    }

    public RISelectChannelConnector(String location)
    {
    ...
    public void close() throws IOException
    {
        synchronized(this)
        {
            ...
            // Stop the Listener for this connector if it is running.
            ListenerManager.getInstance().stopListener(listenerID);
            System.err.println("Stopped the listener with ID: "+listenerID );
    ...
    public void open() throws IOException {
        synchronized (this) {
            if (_acceptChannel == null) {
                ...
                System.out.println("RISelectChannelConnector:Open returned ReuseAddress Value: " + getReuseAddress());
                _acceptChannel.socket().setReuseAddress(true);

                if (!getLocation().isEmpty()) {
                    try {
                        System.out.println("Opening Location: " + getLocation());
                        URL localURL = new URL(getLocation());
                        InetAddress localaddr = InetAddress.getByName(localURL.getHost());
                        InetSocketAddress addr = getHost() == null ? new InetSocketAddress(getPort()) : new InetSocketAddress(localURL.getHost(), localURL.getPort());
                        listenerID = ListenerManager.getInstance().createServerModeListener(localaddr.getHostAddress(), localURL.getPort(),
                                localaddr.getHostAddress(),
                                localURL.getPort(), (getLocation().startsWith("https") ? true : false), getLocation());
                        System.err.println("Created listener " + getLocation() + " with ID: " + listenerID + "\n" + localaddr.getHostAddress() + "\n" + addr.getPort() + "\n" + localURL.getPort() + "\n");

                        _acceptChannel.socket().bind(addr, getAcceptQueueSize());

                    } catch (TransportException tex) {
                        System.err.println("Unable to create listener: " + tex.getMessage());
                        throw new IOException("Unable to create listener.", tex);
                    }
                } else {
                    InetSocketAddress addr = getHost() == null ? new InetSocketAddress(getPort()) : new InetSocketAddress(getHost(), getPort());
                    try{
                    listenerID = ListenerManager.getInstance().createServerModeListener(addr.getHostString(), getPort(),
                                addr.getHostString(),
                                getPort(), (getLocation().startsWith("https") ? true : false), getLocation());
                    } catch (Exception ex){
                        ex.printStackTrace();
                    }
                    
                    _acceptChannel.socket().bind(addr, getAcceptQueueSize());
                    System.out.println("Opening Location: " + addr);
    ...
    {
        return new SelectChannelEndPoint(channel,selectSet,key, RISelectChannelConnector.this._maxIdleTime);
    }

    /* ------------------------------------------------------------------------------- */
    protected Connection newConnection(SocketChannel channel,final SelectChannelEndPoint endpoint)
    {
        return new RIHttpConnection(RISelectChannelConnector.this,endpoint,getServer())
        {
            /* ------------------------------------------------------------ */
            @Override
