 ...
 */
public class DlFullEventUpdateRequest extends DialogProcessor {

    public DlFullEventUpdateRequest() {
        this.setDialogName("dlFullEventUpdateRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.EVENTREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlFullEventUpdateRequest dialog.");
        }
        ...
        if (msgTypeVersion!=null && !msgTypeVersion.isEmpty() && msgTypeID !=null && !msgTypeID.isEmpty()) {
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct message-type-id and message-type-version values for the dlFullEventUpdateRequest dialog.");
        }
