 ...
 */
public class DlCCTVControlRequest extends DialogProcessor {

    public DlCCTVControlRequest() {
        this.setDialogName("dlCCTVControlRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.CCTVCONTROLREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlCCTVControlRequest dialog.");
        }
        
        if(!requestMessage.elementExistsInSpecification(requestMessageType.relatedMessage()+ "." + "cctv-request-command")){
            throw new TMDDMissingInformationError("The message received did not contain a cctv-request-command value for the dlCCTVControlRequest dialog.");
    ...
    protected MessageSpecification generateResponse(MessageSpecification requestMessage, EntityEmulationData.EntityDataType entityDataType, EntityDataInformationCollector dataCollector, EntityCommandProcessor commandProcessor, boolean isPublication) throws NoMatchingDataException, EntityEmulationException {
        ...
        MessageSpecification responseMsg = TMDDMessageProcessor.processCCTVControlRequestMsg(requestMessage, commandProcessor);
        return responseMsg;
    }
