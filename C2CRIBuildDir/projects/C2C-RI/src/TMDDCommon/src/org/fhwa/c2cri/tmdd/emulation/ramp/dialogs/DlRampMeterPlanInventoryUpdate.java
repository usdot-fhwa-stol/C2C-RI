 ...
 */
public class DlRampMeterPlanInventoryUpdate extends DialogProcessor {

    public DlRampMeterPlanInventoryUpdate() {
        this.setDialogName("dlRampMeterPlanInventoryUpdate");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlRampMeterPlanInventoryUpdate dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("4") || msgDeviceInformationType.equals("device plan"))
                && (msgDeviceType.equals("8") || msgDeviceType.equals("ramp meter"))) {

        } else {
            System.out.println("DlRampMeterPlanInventoryUpdate::preprocessing:  msgDeviceType = "+msgDeviceType);
            System.out.println("DlRampMeterPlanInventoryUpdate::preprocessing:  msgDeviceInformationType = "+msgDeviceInformationType);
            for (String thisElement : requestMessage.getMessageSpec()){
                System.out.println(thisElement);
            }
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlRampMeterPlanInventoryUpdate dialog.");
        }
    }
