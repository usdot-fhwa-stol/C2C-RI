 ...
 * </source></pre>
 *
 * @jameleon.function name="C2CRI-session"
 */
public class C2CRISessionTag extends SessionTag {
        ...
        if ((getAppStd() != null)&&(getInfoStd()!=null)) {
            ...
            try {
                    ...
                    try {
                        ...
                        throw new JameleonScriptException("Error Creating Application Layer Standard ->" + ex.getMessage(), this);
                    }
                    ...
                    try{
                        ...
                        ex.printStackTrace();
                        log.debug("*C2CRISessionTag: Error Creating Information Layer Standard ->" + ex.getMessage());
                        // Ensure that the Application Layer Services are stopped.
                        theApplicationLayerStandard.stopServices();
                        theApplicationLayerStandard = null;
                        throw new JameleonScriptException("Error Creating Information Layer Standard ->" + ex.getMessage(), this);

                    }
                
                    theInformationLayerStandard.initializeStandard(testCaseIdentifier, riMode, theApplicationLayerStandard);
                MessageManager theManager = MessageManager.getInstance();
                theManager.setParentTestCase(testCaseIdentifier);

            } catch (Exception ex) {
                log.debug("*C2CRISessionTag: "+ex.getMessage());
//                JOptionPane.showMessageDialog(null, "Script Error: \n" + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                throw new JameleonScriptException("*C2CRISessionTag: "+ ex.getMessage(), this);
            }
    ...
    public void tearDownSession() {
        try {
            ...
            } else if (theApplicationLayerStandard != null){
                theApplicationLayerStandard.stopServices();
                theApplicationLayerStandard = null;
            }
        } catch (Exception ex) {
