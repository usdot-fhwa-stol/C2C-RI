...
import org.fhwa.c2cri.tmdd.emulation.cav.dialogs.DlCenterActiveVerificationRequest;
import org.fhwa.c2cri.tmdd.emulation.cav.dialogs.DlCenterActiveVerificationSubscription;
import org.fhwa.c2cri.tmdd.emulation.cav.dialogs.DlCenterActiveVerificationUpdate;
...
import static org.fhwa.c2cri.tmdd.emulation.entitydata.EntityEmulationData.EntityDataType.CENTERACTIVEVERIFICATION;
 ...
 */
public class CenterActiveVerificationHandler implements EntityControlRequestStatusCollector, EntityCommandProcessor {

    /**
     * The inventory of CCTV devices
     */
    private CenterActiveVerification cav = new CenterActiveVerification(EntityEmulationData.EntityDataType.CENTERACTIVEVERIFICATION);
    /**
    ...
    private CenterActiveVerificationHandler() {
    }
     ...
     */
    public CenterActiveVerificationHandler(EntityDataOrganizationInformationCollector orgInfoDataCollector) {
    ...
    public void initialize(EntityEmulationData.EntityDataType entityDataType, byte[] byteArray) throws EntityEmulationException {
        switch (entityDataType) {
            case CENTERACTIVEVERIFICATION:
                cav.initialize(byteArray);
                break;
            default:
                throw new EntityEmulationException("The CenterActiveVerificationHandler is unable to initialize data type " + entityDataType);
        }
    }
...
//        try {
        ...
        switch (dialog) {
            case "dlCenterActiveVerificationRequest":
                DlCenterActiveVerificationRequest dlCenterActiveVerificationRequestProcessor = new DlCenterActiveVerificationRequest();
                responseMsg = dlCenterActiveVerificationRequestProcessor.handle(requestMessage, EntityEmulationRequests.EntityRequestMessageType.CENTERACTIVEVERIFICATIONREQUESTMSG, TMDDEntityType.EntityType.CENTERACTIVEVERIFICATION, CENTERACTIVEVERIFICATION, cav, this);
                break;
...
//        try {
        switch (dialog) {
            case "dlCenterActiveVerificationSubscription":
                        DlCenterActiveVerificationSubscription dlCenterActiveVerificationProcessor = new DlCenterActiveVerificationSubscription();
                        responseMsg = dlCenterActiveVerificationProcessor.handle(requestMessage, EntityEmulationRequests.EntityRequestMessageType.CENTERACTIVEVERIFICATIONREQUESTMSG, TMDDEntityType.EntityType.CENTERACTIVEVERIFICATION, EntityEmulationData.EntityDataType.CENTERACTIVEVERIFICATION, cav, this);
            break;
            default:
                TMDDErrorResponseProcessor errorProcessor = new TMDDErrorResponseProcessor();
                responseMsg = errorProcessor.getErrorResponseMsgSpec("errorReportMsg", "other", "Dialog " + dialog + " is not currently supported by the CenterActiveVerificationHandler.");
    ...
    public MessageSpecification getPubResponse(String dialog, MessageSpecification requestMessage) {
        ...
        switch (dialog) {
            case "dlCenterActiveVerificationUpdate":
                DlCenterActiveVerificationUpdate dlCenterActiveVerificationUpdateProcessor = new DlCenterActiveVerificationUpdate();
                responseMsg = dlCenterActiveVerificationUpdateProcessor.handle(requestMessage, EntityEmulationRequests.EntityRequestMessageType.CENTERACTIVEVERIFICATIONREQUESTMSG, TMDDEntityType.EntityType.CENTERACTIVEVERIFICATION, CENTERACTIVEVERIFICATION, cav, this);
                break;

            default:
                TMDDErrorResponseProcessor errorProcessor = new TMDDErrorResponseProcessor();
                responseMsg = errorProcessor.getErrorResponseMsgSpec("errorReportMsg", "other", "Dialog " + dialog + " is not currently supported by the CenterActiveVerificationHandler.");
//                    throw new EntityEmulationException("The CenterActiveVerificationHandler is unable to process dialog " + dialog);
            }
    ...
    public MessageSpecification getEntityOrganizationInformation(String entityId) throws NoMatchingDataException, EntityEmulationException {
        ...
        ArrayList<EntityDataRecord> theResponse = cav.getEntityElements(theFilters);
    ...
    public void addEntity(EntityEmulationData.EntityDataType entityDataType, String entityId) throws DuplicateEntityIdException, InvalidEntityIdValueException {
        ...
        try {
            switch (entityDataType) {
                case CENTERACTIVEVERIFICATION:
                    cav.addEntity(entityId);
                    RIEmulation.getInstance().operationRelatedDataUpdate("dlCenterActiveVerificationUpdate");
                    break;
                default:
    ...
    public void addEntityElement(EntityEmulationData.EntityDataType entityDataType, String entityId, String elementName, String elementValue) throws InvalidEntityIdValueException, InvalidValueException {
        try {
            switch (entityDataType) {
                case CENTERACTIVEVERIFICATION:
                    cav.addEntityElement(entityId, elementName, elementValue);
                    RIEmulation.getInstance().operationRelatedDataUpdate("dlCenterActiveVerificationUpdate");
                    break;
                default:
                    throw new EntityEmulationException("The CenterActiveVerificationHandler is unable to process data type " + entityDataType);
    ...
    public void updateEntityElement(EntityEmulationData.EntityDataType entityDataType, String entityId, String entityElementName, String entityElementValue) throws InvalidEntityIdValueException, InvalidValueException {
        try {
            switch (entityDataType) {
                case CENTERACTIVEVERIFICATION:
                    cav.updateEntityElement(entityId, entityElementName, entityElementValue);
                    RIEmulation.getInstance().operationRelatedDataUpdate("dlCenterActiveVerificationUpdate");
                    break;
                default:
                    throw new EntityEmulationException("The CenterActiveVerificationHandler is unable to process data type " + entityDataType);
    ...
    public void deleteEntityElement(EntityEmulationData.EntityDataType entityDataType, String entityId, String entityElementName) throws InvalidEntityIdValueException, InvalidEntityElementException {
        try {
            switch (entityDataType) {
                case CENTERACTIVEVERIFICATION:
                    cav.deleteEntityElement(entityId, entityElementName);
                    RIEmulation.getInstance().operationRelatedDataUpdate("dlCenterActiveVerificationUpdate");
                    break;
                default:
                    throw new EntityEmulationException("The CenterActiveVerificationHandler is unable to process data type " + entityDataType);
    ...
    public String getEntityElementValue(EntityEmulationData.EntityDataType entityDataType, String entityId, String entityElement) throws InvalidEntityIdValueException, InvalidEntityElementException, EntityEmulationException {
        ...
        try {
            switch (entityDataType) {
                case CENTERACTIVEVERIFICATION:
                    entityValue = cav.getEntityElementValue(entityId, entityElement);
                    break;
                default:
                    throw new EntityEmulationException("The CenterActiveVerificationHandler is unable to process data type " + entityDataType);
            }
