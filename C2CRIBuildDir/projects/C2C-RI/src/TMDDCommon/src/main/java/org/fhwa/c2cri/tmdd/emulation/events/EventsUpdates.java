 ...
 */
public class EventsUpdates extends TMDDDefaultEntityDataType{

    public EventsUpdates(EntityEmulationData.EntityDataType entityDataType){
    ...
    public String getMatchingOrNextEntityIdEntry(EntityEmulationData.EntityDataType entityDataType, String entityId){
        ...
        String relatedEventId = getEventsUpdatesEventId(entityId);
        ...
        try{
            ...
            if (entityDataType.equals(EntityEmulationData.EntityDataType.EVENTSACTIONLOG)){
                response = nextValue.toString();                
            } else{
...
//            ex.printStackTrace();
        ...
        System.out.println("    EventsUpdates:  "+entityDataType.name() + " index used = "+response + "  computed next value was "+nextValue);
        return response;
    }

    
    public String getEventsUpdatesEventId(String entityId){
        String results = "";
        
        try {
           ArrayList<String> entityIdFilterValues = new ArrayList();
            entityIdFilterValues.add(entityId);
            ValueInSetFilter entityIdFilter = new ValueInSetFilter(EntityEmulationData.EntityDataType.EVENTSUPDATES, "EntityId", entityIdFilterValues);
            ArrayList<DataFilter> entityFilters = new ArrayList();
            entityFilters.add(entityIdFilter);
            ArrayList<EntityDataRecord> eventResults = EntityEmulationData.getRelatedEntityElementsWithValueType(EntityEmulationData.EntityDataType.EVENTSUPDATES, entityFilters,"EventId");
            if (eventResults.size() > 0){
                return (eventResults.get(0)).getEntityElementValue();
            }
        } catch (Exception ex){
            System.out.println(" There are no current events associated with "+EntityEmulationData.EntityDataType.EVENTSUPDATES+" " + entityId);
        }
        return results;
    }
}
