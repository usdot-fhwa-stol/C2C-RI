 ...
 */
public class DlESSObservationReportRequest extends DialogProcessor {

    public DlESSObservationReportRequest() {
        this.setDialogName("dlESSObservationReportRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlESSObservationReportRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("6") || msgDeviceInformationType.equals("device data"))
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlESSObservationReportRequest dialog.");
        }
    }
