 ...
 */
public class ArchiveDataProcessingDocumentationMetaData extends TMDDDefaultEntityDataType{

    public ArchiveDataProcessingDocumentationMetaData(EntityEmulationData.EntityDataType entityDataType){
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArchiveDataProcessingDocumentationMetaData cctv = new ArchiveDataProcessingDocumentationMetaData(EntityEmulationData.EntityDataType.ARCHIVEDDATAPROCESSINGDOCUMENTATIONMETADATA);

        System.out.println(EmulationDataFileProcessor.getContent(EntityEmulationData.EntityDataType.ARCHIVEDDATAPROCESSINGDOCUMENTATIONMETADATA + ".out").toString());

        if (!readFile) {
            cctv.initialize(null);
        } else {
            cctv.initialize(EmulationDataFileProcessor.getByteArray(EntityEmulationData.EntityDataType.ARCHIVEDDATAPROCESSINGDOCUMENTATIONMETADATA + ".out"));
        }
        ...
        ValueInSetFilter firstFilter = new ValueInSetFilter(EntityEmulationData.EntityDataType.ARCHIVEDDATAPROCESSINGDOCUMENTATIONMETADATA, "EntityId", filterValues);
        theFilters.add(firstFilter);
        results = cctv.getEntityElements(theFilters);
        System.out.println("Records Returned = " + results.size());

        if (writeFile) {
            EntityDataFile.writeFile(EntityEmulationData.EntityDataType.ARCHIVEDDATAPROCESSINGDOCUMENTATIONMETADATA + ".out", results);
        }
    }
}
