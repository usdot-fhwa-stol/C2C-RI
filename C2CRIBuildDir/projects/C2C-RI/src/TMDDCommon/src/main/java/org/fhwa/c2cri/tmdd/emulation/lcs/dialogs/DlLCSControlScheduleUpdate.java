 ...
 */
public class DlLCSControlScheduleUpdate extends DialogProcessor {

    public DlLCSControlScheduleUpdate() {
        this.setDialogName("dlLCSControlScheduleUpdate");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlLCSControlScheduleUpdate dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("3") || msgDeviceInformationType.equals("device schedule"))
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlLCSControlScheduleUpdate dialog.");
        }
    }
