 ...
 */
public class DlNodeStatusRequest extends DialogProcessor {

    public DlNodeStatusRequest() {
        this.setDialogName("dlNodeStatusRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.TRAFFICNETWORKINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlNodeStatusRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("2") || msgDeviceInformationType.equals("node status"))){

        } else {
            throw new TMDDMissingInformationError("The message received did not contain a correct network-information-type values for the dlNodeStatusRequest dialog.");
        }
    }
