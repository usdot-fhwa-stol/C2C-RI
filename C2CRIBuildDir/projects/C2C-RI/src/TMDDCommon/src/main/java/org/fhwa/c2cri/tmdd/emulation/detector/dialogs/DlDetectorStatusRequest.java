 ...
 */
public class DlDetectorStatusRequest extends DialogProcessor {

    public DlDetectorStatusRequest() {
        this.setDialogName("dlDetectorStatusRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlDetectorStatusRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("2") || msgDeviceInformationType.equals("device status"))
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlDetectorStatusRequest dialog.");
        }
    }
