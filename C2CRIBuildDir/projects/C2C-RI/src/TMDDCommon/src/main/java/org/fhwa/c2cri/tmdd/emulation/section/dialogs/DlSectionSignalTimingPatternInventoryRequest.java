 ...
 */
public class DlSectionSignalTimingPatternInventoryRequest extends DialogProcessor {

    public DlSectionSignalTimingPatternInventoryRequest() {
        this.setDialogName("dlSectionSignalTimingPatternInventoryRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.SECTIONSIGNALTIMINGPATTERNINVENTORYREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlSectionSignalTimingPatternInventoryRequest dialog.");
        }
