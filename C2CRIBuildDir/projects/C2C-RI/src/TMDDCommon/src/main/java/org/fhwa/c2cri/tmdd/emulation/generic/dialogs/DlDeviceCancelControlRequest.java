 ...
 */
public class DlDeviceCancelControlRequest extends DialogProcessor {

    public DlDeviceCancelControlRequest() {
        this.setDialogName("dlDeviceCancelControlRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICECANCELCONTROLREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlDeviceCancelControlRequest dialog.");
        }
...
//        MessageSpecification responseMsg = TMDDMessageProcessor.processDeviceCancelControlRequestMsg(requestMessage, entityDataType, dataCollector, false);
        MessageSpecification responseMsg = TMDDMessageProcessor.processDeviceCancelControlRequestMsg(requestMessage, commandProcessor);
        return responseMsg;
    }
