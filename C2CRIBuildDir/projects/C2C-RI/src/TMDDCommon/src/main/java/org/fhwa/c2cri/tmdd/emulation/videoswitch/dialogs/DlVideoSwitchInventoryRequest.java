 ...
 */
public class DlVideoSwitchInventoryRequest extends DialogProcessor {

    public DlVideoSwitchInventoryRequest() {
        this.setDialogName("dlVideoSwitchInventoryRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlVideoSwitchInventoryRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("1") || msgDeviceInformationType.equals("device inventory"))
                && (msgDeviceType.equals("11") || msgDeviceType.equals("video switch"))) {

        } else {
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlVideoSwitchInventoryRequest dialog.");
        }
    }
