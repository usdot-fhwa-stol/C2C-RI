 ...
 */
public class DlIntersectionSignalControlScheduleRequest extends DialogProcessor {

    public DlIntersectionSignalControlScheduleRequest() {
        this.setDialogName("dlIntersectionSignalControlScheduleRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlIntersectionSignalControlScheduleRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("3") || msgDeviceInformationType.equals("device schedule"))
                && (msgDeviceType.equals("9") || msgDeviceType.equals("signal controller"))) {

        } else {
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlIntersectionSignalControlScheduleRequest dialog.");
        }
    }
