 ...
 */
public class DlRampMeterPlanInventoryRequest extends DialogProcessor {

    public DlRampMeterPlanInventoryRequest() {
        this.setDialogName("dlRampMeterPlanInventoryRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlRampMeterPlanInventoryRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("4") || msgDeviceInformationType.equals("device plan"))
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlRampMeterPlanInventoryRequest dialog.");
        }
    }
