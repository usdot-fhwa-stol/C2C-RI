 ...
 */
public class DlHARInventoryUpdate extends DialogProcessor {

    public DlHARInventoryUpdate() {
        this.setDialogName("dlHARInventoryUpdate");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlHARInventoryUpdate dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("1") || msgDeviceInformationType.equals("device inventory"))
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlHARInventoryUpdate dialog.");
        }
    }
