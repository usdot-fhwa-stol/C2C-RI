 ...
 */
public class DlDetectorDataSubscription extends DialogProcessor {

    public DlDetectorDataSubscription() {
        this.setDialogName("dlDetectorDataSubscription");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlDetectorDataSubscription dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("6") || msgDeviceInformationType.equals("device data"))
            ...
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlDetectorDataSubscription dialog.");
        }
    }
