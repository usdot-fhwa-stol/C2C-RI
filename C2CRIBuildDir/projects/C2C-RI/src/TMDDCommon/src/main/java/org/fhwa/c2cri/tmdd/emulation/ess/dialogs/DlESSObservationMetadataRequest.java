 ...
 */
public class DlESSObservationMetadataRequest extends DialogProcessor {

    public DlESSObservationMetadataRequest() {
        this.setDialogName("dlESSObservationMetadataRequest");
    ...
    protected void preprocessing(EntityEmulationRequests.EntityRequestMessageType requestMessageType, MessageSpecification requestMessage) throws TMDDCenterDoesNotSupportMessageError, TMDDMissingInformationError {
        if (!requestMessage.containsMessageOfType(EntityEmulationRequests.EntityRequestMessageType.DEVICEINFORMATIONREQUESTMSG.relatedMessage())) {
            throw new TMDDCenterDoesNotSupportMessageError("The message received did not contain the correct message type for the dlESSObservationMetadataRequest dialog.");
        }
        ...
        if ((msgDeviceInformationType.equals("7") || msgDeviceInformationType.equals("device metadata"))
                && (msgDeviceType.equals("4") || msgDeviceType.equals("environmental sensor station"))) {

        } else {
            throw new TMDDMissingInformationError("The message received did not contain correct device-type and device-information-type values for the dlESSObservationMetadataRequest dialog.");
        }
    }
